use seekdb_rs::{AdminApi, AdminClient, DistanceMetric, EmbeddingFunction, HnswConfig, SeekDbError, ServerClient, Embedding, Metadata};
use std::sync::Arc;
mod common;
...
async fn test_readme_doc() -> Result<(), SeekDbError> {
    ...
    let coll = client.get_or_create_collection(
        "test_readme", 
        Some(hnsw), 
    ...
    let coll_test_readme = client.get_collection("test_readme", None::<Box<dyn EmbeddingFunction>>).await?;
    println!("Collection {:?} created successfully", coll_test_readme.name());
    ...
    println!("Collection {:?} count: {:?}", coll_test_readme.name(), cnt_coll);
    ...
    coll.add(&ids, Some(&embeddings), Some(&metadatas), Some(&documents)).await?;
    coll.update(
        &["item1".to_string()],
    ...
    let r = coll.get(Some(&["item1".to_string()]), None, None, None, None, None).await?;
    println!("{:?}", r);
    
    coll.delete(Some(&["item1".to_string(), "item2".to_string()]), None, None).await?;
